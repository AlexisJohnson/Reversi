...
plays = []  # so we don't have to search 700+ nodes at runtime
...
class Node():
    ...
    def __init__(self, myBoard, myTile, a, parent):
...
def UCT(x):
    ...
    if x not in visits:
        visits.add(x)
    if getValidMoves(x.myBoard, x.myTile) == []:  # reached terminal node, no plays left
        ...
        v, a2 = UCT(y)
    # backpropogate the reward and times visited up to the root
    x.r = (x.r*x.t + v)/(x.t + 1)
    x.t = x.t + 1
    return v, a
...
def generateChildren(x):
    ...
    if (len(x.children) == 0):
        ...
        for a in getValidMoves(x.myBoard, x.myTile):
            ...
            x.children.append(Node(newBoard, newTile, a, None))
...
def UCBchoose(x):
    ...
    """
    # if first time visiting. we add children to node to retain their values
    generateChildren(x)
    Y = list(set(x.children) - visits)

    # exploratory phase. All children of parent should be explored once before
    if (len(Y) > 0):
    ...
    # UCB alg
    if x.myTile == 'X':  # max
        Q = list(map(lambda y: y.r + sqrt((2 * log(t))/y.t) - greed(y), x.children))
        i = np.argmax(Q)
        return x.children[i], x.children[i].a
    else:  # min
        Q = list(map(lambda y: 1-y.r + sqrt((2 * log(t))/y.t) - greed(y), x.children))
        i = np.argmax(Q)
        return x.children[i], x.children[i].a
...
def greed(x):
    """
    Heuristic used to minimize the other player's possible moves
    by punishing the agent for higher scores early on in the game
    (player i's score - player -i's score)/(player i's score + player -i's score)^b.
    Subtracted from Q val
        Parameter:
            x (Node): node obj
        Returns:
            penalty (float): punishment

    src: https://royhung.com/reversi
    """
    advTile = 'X' if x.myTile == 'O' else 'O'
    score = getScoreOfBoard(x.myBoard)[x.myTile]
    advScore = getScoreOfBoard(x.myBoard)[advTile]
    penalty = (score - advScore)/pow((score + advScore), 2.23)
...
def get_move(board, tile):
    ...
    possibleMovesTup = set(map(lambda x: tuple(x), possibleMoves))
    myBoard = copy.deepcopy(board)
    cornerActions = corners.intersection(possibleMovesTup)

    if len(cornerActions) > 0:
        # always chooses corner when available
        return list(cornerActions.pop())
    if ((len(plays) == 0) or (plays[-1].myBoard != myBoard)):
        state = Node(myBoard, tile, None, None)
        plays.append(state)
        UCT(state)
    else:
        state = plays[-1]
